...
export class HomePage {
  ...
  printTicket(ticketData: any[]) {
    ...
    if (!qz.websocket.isActive()) {
      ...
      qz.websocket.connect().then(function () {
        ...
        const config = qz.configs.create(printers[3]);
  ...
  printLastTicket(ticket) {
    ...
    let ticketDetails = '';
    const code = ticket.ticketNo;
    let bets = '';
    let totalAmt = 0;
    // tslint:disable-next-line: max-line-length
    const drawDateTime = this.dateFunction.convertStringDate(this.utility.convertNoIntoDate(ticket.drawtime), 'dd-MM-yyyy hh:mm a');

    /** Make String of Number and corresponding entered amount
     * and calculate total Amount
     */
    const betArr: any[] = ticket.selectedNumbers;
      ...
      (result) => {
        ...
        if (result.status === 200) {
          ...
          if (result != null) {
            ...
            const ticketDetails = '\x1B' + '\x45' + '\x0D' + 'KuberLaxmi Lottery(Goa)\n' + '\x1B' + '\x45' + '\x0A'
              + 'Four Digit Online Weekly\n'
              + 'Dr. ' + drawDateTime + '\n'
              + '\x1D' + '\x21' + '\x01' + bets + '\x1D' + '\x21' + '\x00'
              + 'Qty : ' + this.ticketArr.length + ' Total : Rs. ' + totalAmt + '\n'
  ...
  async getLastFiveTickets() {
    ...
    const postData = '?ticket_no=' + '';

    // Clear array
    this.lastFiveTicketArr = [];
    this.isShowLastFiveTicketModel = false;

    this.apiService.getApi(this.appConstants.getLastFiveTicketsUrl).subscribe(
      (result) => {
        // console.log(JSON.stringify(result));
        if (result.status === 200) {
          ...
          if (result.body != null) {
            ...
            this.lastFiveTicketArr = result.body;
  ...
  async getTicketDetails() {
    ...
    const postData = '?ticket_no=' + this.barcodeNo;


    this.apiService.getApi(this.appConstants.getTicketDetailsUrl+postData).subscribe(
      (result) => {
        // console.log(JSON.stringify(result));
        if (result.status === 200) {
          ...
          if (result.body > 0) {
            ...
            if (!this.utility.checkStringNullEmpty(data) && parseInt(data) > 0) {
              this.isUserWin = true;
              this.winingAmount = data;
      ...
      (result) => {
        if (result.status === 200) {
          ...
          if (result.body != null) {
            ...
            setInterval(() => {
              this.time = this.datePipe.transform(new Date(), 'dd-MM-yyyy hh:mm:ss a');
            }, 1000);
  ...
  async cancelTicketByUser(ticketNo: any) {
    ...
    const postData = 'ticket_no=' + ticketNo;
